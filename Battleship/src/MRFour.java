...
public class MRFour implements Captain, Constants {
    ...
    int[] shipLengths;
    boolean hitShips;
    int smallestShip, lastX, lastY, numRounds;
    LinkedList<square> queue;
    HashMap<Integer, Integer> remainingShips;
    HashMap<String, int[][]> opponents;
    int[][] currentOpponent;
    ...
    public void initialize(int numMatches, int numCaptains, String opponent) {
        ...
        if (!opponents.containsKey(opponent)) {
            opponents.put(opponent, new int[10][10]);
        }
    ...
    public Coordinate makeAttack() {
        ...
        if (!hitShips) {
            ...
            for (square s : queue) {
                values[s.x][s.y] = s.getValue(remainingShips);
    ...
    public void opponentAttack(Coordinate coord) {
        currentOpponent[coord.getX()][coord.getY()]++;
    }
    ...
    void placeShips() {
        ...
        minValue = currentOpponent[0][0] + currentOpponent[0][1];
        patrol.add(new int[]{0, 0, VERTICAL, minValue});
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 9; j++) {
                thisValue = currentOpponent[i][j] + currentOpponent[i][j + 1];
                ...
                thisValue = currentOpponent[j][i] + currentOpponent[j + 1][i];
                if (thisValue <= minValue * 21 / 20) {
                    if (thisValue < minValue) {
                        for (Iterator<int[]> it = patrol.iterator(); it.hasNext();) {
                            int[] info = it.next();
