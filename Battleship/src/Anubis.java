 ...
 * ~Anubis~ The god risen from the unfinished god Sobek - come back with even more unfinishedness.
 * -Brian Dassow Jr. Spring 2015-
 */

public class Anubis implements Captain, Constants {
	...
	public final int GAME_THRESHOLD_TILL_CHECK = 1000000;
	public final int GAME_THRESHOLD_TILL_RESET = 50000000;
	public final int GAME_THRESHOLD_TILL_SAMPLE = 50000000;
	...
	boolean[] shipSunk, seeker, shipHOrVGuessed,
	...
	public void initialize(int numMatches, int numCaptains, String opponent) { 
		...
		if (!myOpponent.name.equals(opponent)) {
			...
			//allPlacementMethods[2] = new PlacementMethod("AdaptiveStat");

			
			//I SHOULD ADD ONE THAT DOES 4 + 2 ..duh also make sure they are attacking the same square.
	...
	public void placeShipAdaptive(int shipID) {
		...
		while (!shipPlaced) {
			int orientation = generator.nextInt(2);
			if (orientation == VERTICAL && theirAttackHash.get(count).y + SHIP_HIT_IDS[shipID] > 9) {
				if (myFleet.placeShip(theirAttackHash.get(count).x, theirAttackHash.get(count).y - SHIP_LENGTHS[shipID] + 1, orientation, shipID)) {
					shipPlaced = true;
				}
				count--;
			} else if (orientation == HORIZONTAL && theirAttackHash.get(count).y + SHIP_HIT_IDS[shipID] > 9) {
	...
	public void placeShipAdaptivelyStat(int shipID) {
		BoardCoordinate[] wholeBoard = new BoardCoordinate[100];
		BoardCoordinate tempBoard;
		int total = 0;
		...
		for (int i = 0; i < 100; i++) {
			...
			for (int j = 0; j < SHIP_LENGTHS[shipID]; j++) {
				for (int k = 0; k < 100; k++) {
					if(theirAttackHash.get(i).x == theirAttackHash.get(k).x+j && theirAttackHash.get(i).x == theirAttackHash.get(k).y) {
						total += theirAttackHash.get(k).score;
					}
					
					if(theirAttackHash.get(i).x == theirAttackHash.get(k).x && theirAttackHash.get(i).x == theirAttackHash.get(k).y+j) {
						total += theirAttackHash.get(k).score;
					}
				}
			}
			tempBoard = new BoardCoordinate(theirAttackHash.get(i).x, theirAttackHash.get(i).y, total);
			wholeBoard[i] = tempBoard;
		}
		
		insertionSort(wholeBoard, wholeBoard.length);
		int counter = 99;
		
		while (!myFleet.placeShip(wholeBoard[counter].x, wholeBoard[counter].y, generator.nextInt(2), shipID)) {
			counter--;
		}
